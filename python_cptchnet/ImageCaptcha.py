...
class ImageCaptcha:
    ...
    def __init__(
        ...
        img_path: str = "PythonCptchNetImages",
        **kwargs,
    ):
        """
        ...
        :param kwargs: Служит для передачи необязательных параметров в пайлоад для запроса к Cptch.net
    ...
    def __image_temp_saver(self, content: bytes):
        ...
        captcha_id = None
        try:
            # Отправляем на капчу изображение капчи и другие парметры,
    ...
    def __local_image_captcha(self, content: str, content_type: str = "file"):
        """
        Метод получает в качестве параметра ссылку на локальный файл(или файл в кодировке base64), считывает изображение
            и отправляет его на Капчу
        ...
        try:
            # пробуем открыть файл, закодировать в base64, затем вносим закодированный файл в post_payload
            ...
            # вносим закодированный файл в post_payload для отправки на сервис для решения
    ...
    def captcha_handler(
        ...
        **kwargs,
        ...
        Cptch.net, дожидается решения капчи и вовзращает вам результат
        ...
        """
        # result, - задаётся в декораторе `service_check`, после проверки переданного параметра
